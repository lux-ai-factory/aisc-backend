...
from aisc_backend.routers.model import router, UploadModelFileResponse
...
async def upload_model_file(pid: uuid.UUID) -> UploadModelFileResponse:
    with (
        patch("aisc_backend.routers.model.file_repository.upload_file", new_callable=MagicMock) as mock_upload_response,
    ):
