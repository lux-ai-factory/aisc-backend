...
from aisc_backend.routers.dataset import router, UploadDatasetFileResponse
...
async def upload_dataset_file(pid: uuid.UUID) -> UploadDatasetFileResponse:
    with (
        patch("aisc_backend.routers.dataset.file_repository.upload_file",
              new_callable=MagicMock) as mock_upload_response,
        patch("aisc_backend.routers.dataset.aisc_eval.autodiscover_datashape", new_callable=AsyncMock) as eval_response,
    ):
...
async def get_dataset_file_data(pid: uuid.UUID) -> StreamingHttpResponse:
    with (
        patch("aisc_backend.routers.dataset.file_repository.bucket_exists", new_callable=MagicMock) as mock_bucket_exists_response,
        patch("aisc_backend.routers.dataset.file_repository.get_object", new_callable=MagicMock) as mock_get_object_response,
    ):
