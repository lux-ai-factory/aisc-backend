...
from vera_backend.routers.dataset import router, UploadDatasetFileResponse
...
async def upload_dataset_file(pid: uuid.UUID) -> UploadDatasetFileResponse:
    with (
        patch("vera_backend.routers.dataset.file_repository.upload_file",
              new_callable=MagicMock) as mock_upload_response,
        patch("vera_backend.routers.dataset.vera_eval.autodiscover_datashape", new_callable=AsyncMock) as eval_response,
    ):
