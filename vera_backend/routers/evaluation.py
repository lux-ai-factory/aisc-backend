...
async def create_evaluation_measures(request, evaluation_pid: uuid.UUID, data: dict[uuid.UUID, list[MeasureInSchema]] = Body(...)):
    evaluation = await evaluation_repository.get(evaluation_pid, True)
...
async def get_evaluation_artifacts(request, evaluation_pid: uuid.UUID, evaluation_plugin_uuid: uuid.UUID):
